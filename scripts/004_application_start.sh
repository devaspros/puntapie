...
echo "$(date '+%F %T') rsyncing release folder with app folder" >> $LOG_DIR/008_rsync_files.log 2>&1
rsync -arv --delete-after \
  --exclude "vendor/" \
  $DEPLOY_DIR/ \
  $BASE_DIR/app/ \
  >> $LOG_DIR/008_rsync_files.log 2>&1

echo "$(date '+%F %T') Reiniciando aplicación (touch tmp/restart.txt)" >> $LOG_DIR/009_nginx_restart.log 2>&1
touch ./tmp/restart.txt >> $LOG_DIR/009_nginx_restart.log 2>&1

if [ -f "log/$RAILS_ENV.log" ]; then
  chmod 664 "log/$RAILS_ENV.log"
fi

echo "$(date '+%F %T') Changing ownership to ubuntu" >> $LOG_DIR/010_chowning.log 2>&1
cat ~/.clave | sudo chown -Rv ubuntu:ubuntu log tmp vendor >> $LOG_DIR/010_chowning.log 2>&1
